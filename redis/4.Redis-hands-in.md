...
실습을 위해서 개인별로 Bastion server 와 Redis Key Prefix 를 부여 받았을 것이다.

본 문서에서는 01 (예를들면 user02) 번으로 설정한 script 들이 존재한다.

개인별 실습을 원할히 하기 위해서 아래 사항들을 변경하자.

typora 를 사용하고 있다면 아래처럼 Ctrl+H 명령으로 일괄 변경하자.



## 1) Bastion Server IP 변경

개인별 Bastion Server IP 로 변경하자.

```
Ctrl + H

3.36.88.60 -->  (자신의 Bastion Server IP)    > 모두 변경
...
* 일반적으로 Kubernetes 에 서비스를 배포하기 위해 준비되는 Manifest 파일은 정적인 형태이다. 
* 따라서 데이터를 수정하기 위해선 파일 자체를 수정해야 한다.  잘 관리를 한다면 큰 어려움은 없겠지만, 문제는 CI/CD 등 자동화된 파이프라인을 구축해서 애플리케이션 라이프사이클을 관리할 때 발생한다.  
* 보통 애플리케이션 이미지를 새로 빌드하게 되면, 빌드 넘버가 변경된다. 
* 이렇게 되면 새로운 이미지를 사용하기 위해 Kubernetes Manifest의 Image도 변경되어야 한다.  
* 하지만 Kubernetes Manifest를 살펴보면, 이를 변경하기 쉽지 않다. 
* Image Tag가 별도로 존재하지 않고 Image 이름에 붙어있기 때문입니다. 
* 이를 자동화 파이프라인에서 변경하려면, sed 명령어를 쓰는 등의 힘든 작업을 해야 한다.
* Image Tag는 굉장히 단적인 예제이다.  
* 이 외에 도 Configmap 등 배포시마다 조금씩 다른 형태의 데이터를 배포해야 할때 Maniifest 파일 방식은 너무나 비효율적이다.  
...
# 2024.03.14
bitnami/redis                   18.19.2         7.2.4           Redis(R) is an open source, advanced key-value ...
bitnami/redis-cluster           9.8.1           7.2.4           Redis(R) is an open source, scalable, distribut...
...
$ ll
-rw-r--r-- 1 song song 93712 Mar 14 22:41 redis-cluster-9.8.1.tgz
...
$ tar -xzvf redis-cluster-9.8.1.tgz
...
...
$ redis-cli -h ds-redis-redis-cluster -p 6379 -c -a new1234
...
$ nc -zv ds-redis-redis-cluster 6379
...
-rw-r--r-- 1 ubuntu ubuntu 87547 Mar 12 07:37 redis-18.19.0.tgz
-rw-r--r-- 1 song song 87598 Mar 14 22:52 redis-18.19.2.tgz



$ tar -xzvf  redis-18.19.2.tgz
...
...
- 외부에서 접근시에는 nodeport 로 접근 한다.
  - Node_IP:32300     <-- master 
  - Node_IP:32310     <-- replicas
...
# master connect
$ redis-cli -h ds-redis-master -p 6379 -a new1234
...
# 실행
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 15.spt-redis-mr-set.py ds-redis-master.redis-mr.svc 6379 user02 10000
...
# 실행
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 16.spt-redis-mr-get.py ds-redis-master.redis-mr.svc 6379 user02 10000
...
- Bastion Server IP 는 개인별로 부여된 IP 이므로 이를 사용하자.
  ...
  ※ Typora 를 사용하고 있다면 본문서에 등장하는 3.36.88.60 를 모두 자신의 IP 로 Replace 사용하자.
...
$ redis-cli -h 3.36.88.60 -c -a new1234 -p 32300
...
3.36.88.60:32300> get a
"1"
3.36.88.60:32300> get b
"2"
3.36.88.60:32300> get c
"3"


# set 명령 수행
3.36.88.60:32300> set e 1
OK
3.36.88.60:32300> set f 2
OK
3.36.88.60:32300> set g 3
OK
...
# 2) 계정 추가
3.36.88.60:32300> acl setuser supersong on >new1234 allcommands allkeys
OK
3.36.88.60:32300> acl setuser tempsong on >new1234 allcommands allkeys
OK
3.36.88.60:32300>  acl list
1) "user default on sanitize-payload #65fd3b5c243ea857f91daef8e3d5c203fa045f33e034861998b9d74cc42ceb24 ~* &* +@all"
...
# 3) 계정 전환
3.36.88.60:32300> acl whoami
"default"
3.36.88.60:32300> auth supersong new1234
OK
3.36.88.60:32300> acl whoami
"supersong"
3.36.88.60:32300> auth default new1234
OK
3.36.88.60:32300> acl whoami
"default"
...
# 4) 계정 삭제
3.36.88.60:32300> acl deluser tempsong
(integer) 1
3.36.88.60:32300> acl list
...
# 1) 읽기 계정 생성
3.36.88.60:32300> acl setuser readonlysong on >new1234 allcommands allkeys -set +get
OK
3.36.88.60:32300> acl list
1) "user default on #65fd3b5c243ea857f91daef8e3d5c203fa045f33e034861998b9d74cc42ceb24 ~* &* +@all"
...
3.36.88.60:32300> acl whoami
"default"
3.36.88.60:32300> auth readonlysong new1234
OK
...
## 1-1) song 계정 생성
3.36.88.60:32300> acl setuser song on >new1234 allcommands allkeys
OK
...
3.36.88.60:32300> acl setuser song resetkeys ~song*
OK
...
# 2) 계정전환
3.36.88.60:32300> auth song new1234
OK
