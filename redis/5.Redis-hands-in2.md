...
# set
43.203.62.69:32300> set user02:a 1
43.203.62.69:32300> set user02:b 2
43.203.62.69:32300> set user02:c 3


# get
43.203.62.69:32300> get user02:a
"1"
43.203.62.69:32300> get user02:b
"2"
43.203.62.69:32300> get user02:c
"3"
...
43.203.62.69:32310> set user02:a 1
...
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 15.spt-redis-mr-set.py 43.203.62.69 32300 user02 10000
...
[key 1] duration time : 31.04359269142151
[key 2] duration time : 62.13269782066345
[key 3] duration time : 76.54096174240112
...
# 실행
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 16.spt-redis-mr-get.py 43.203.62.69 32300 user02 10000
...
[key 1] duration time : 22.64852023124695
[key 2] duration time : 62.174551248550415
[key 3] duration time : 80.62148237228394
...
# 실행
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 16.spt-redis-mr-get.py 43.203.62.69 32310 user02 10000
...
$ kubectl create ns redis-client-user02

$ kubectl -n redis-client-user02 get pod


# alias 생성
$ alias ku='kubectl -n redis-client-user02'
...
# set
10.42.2.39:6379> set user02:a 1
-> Redirected to slot [10728] located at 10.42.1.56:6379
OK

10.42.1.56:6379> set user02:b 2
OK

10.42.1.56:6379> set user02:c 3
-> Redirected to slot [2474] located at 10.42.2.39:6379
OK


# get
10.42.2.39:6379> get user02:a
-> Redirected to slot [10728] located at 10.42.1.56:6379
"1"

10.42.1.56:6379> get user02:b
"2"

10.42.1.56:6379> get user02:c
-> Redirected to slot [2474] located at 10.42.2.39:6379
"3"
...
# 실행
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 11.spt-cluster-set.py ds-redis-redis-cluster.redis-cluster.svc 6379 user02 10000
...
[key 1] duration time : 16.35304880142212
[key 2] duration time : 40.43312454223633
[key 3] duration time : 60.152461528778076
...
# 실행
$ cd ~/githubrepo/ktds-edu-redis/redis/PythonSample
  python 12.spt-cluster-get.py ds-redis-redis-cluster.redis-cluster.svc 6379 user02 10000
...
[key 1] duration time : 7.3480799198150635
[key 2] duration time : 20.781083345413208
[key 3] duration time : 38.005741119384766
...
43.203.62.69:32300> set user02:count 50
OK

43.203.62.69:32300> incr user02:count
(integer) 51

43.203.62.69:32300> get user02:count
"51"

43.203.62.69:32300> incrby user02:count 100
(integer) 151

43.203.62.69:32300> decr user02:count
(integer) 150

43.203.62.69:32300> decrby user02:count 500
(integer) -350

43.203.62.69:32300> mset user02:a "hello" user02:b "world"
OK

43.203.62.69:32300> mget user02:a user02:b
1) "hello"
2) "world"
...
```sql
Insert into user02(username, email) values ('song', 'song@b.c');
Insert into user02(username, email) values ('lee', 'lee@b.c');
```
...
set user02:song 'song@b.c'
set user02:lee 'lee@b.c'
...
# LPUSH를 통한 list 생성
> LPUSH user02:myList "a"
(integer) 1
> LRANGE user02:myList 0 -1       # list 에 모든 값 확인
1) "a"

# LPUSH , RPUSH를 통한 요소 삽입 결과.
> LPUSH user02:myList "b"
(integer) 2
> RPUSH user02:myList "c"
(integer) 3
> LRANGE user02:myList 0 -1
1) "b"
2) "a"
3) "c"
...
> LPUSHX user02:myList2 "d"
(integer) 0

> LPUSH user02:myList "d"
(integer) 4
...
> LSET user02:myList 0 "z"
OK
> LRANGE user02:myList 0 -1
1) "z"

# RPOPLPUSH 
> RPOPLPUSH user02:myList user02:hello     # myList 에서 rpop 하여 hello 에 lpush 한다.
"z"
> LRANGE user02:hello 0 -1
1) "z"
```
...
> zadd user02:fruit 2 apple
(integer) 1

> zadd user02:fruit 10 banana
(integer) 1

# 복수개의 score-member를 추가할 수 있음
> zadd user02:fruit 8 melon 4 orange 6 watermelon
(integer) 3

# 이미 추가 된 member를 add 시 score가 업데이트
> zadd user02:fruit 15 apple
(integer) 0

# ZSCORE : member에 해당하는 score 값 리턴
> zscore user02:fruit apple
"15"

# ZRANK : member에 해당하는 rank(순위) 값 리턴
> zrank user02:fruit melon
(integer) 2

# ZRANGE : key에 해당하는 start - stop 내가 출력하고 싶은 요소를 추출
> zrange user02:fruit 0 -1
1) "orange"
2) "watermelon"
3) "melon"
4) "banana"
...
> hset user02:hh name song year 1975 month 1
(integer) 3
...
# Insert into user02(id, name, email) values(111, 'song', 'song@b.c');
$ hmset user02:111 name song email song@b.c
...
43.203.62.69:32300> set user02:bit A
OK

43.203.62.69:32300> get user02:bit
"A"

43.203.62.69:32300> getbit  user02:bit 6
(integer) 0

# bit값조정
# 6번째 bit를 1로 변경
43.203.62.69:32300> setbit user02:bit 6 1
(integer) 0
...
# BITCOUNT
43.203.62.69:32300> set user02:bit ABC
OK

43.203.62.69:32300> BITCOUNT user02:bit 1 2
(integer) 5

43.203.62.69:32300> BITCOUNT user02:bit
(integer) 7
...
```sh
> SADD user02:awesome-event:20240310 user_1
(integer) 1 # SET에 새로 추가된 member인 경우 1

> SADD user02:awesome-event:20240310 user_1
(integer) 0 # SET에 이미 존재하는 member인 경우 0
 
> SADD user02:awesome-event:20240310 user_2
(integer) 1

> SADD user02:awesome-event:20240310 user_3
(integer) 1


# 해당 키에 속한 멤버 리턴
> smembers user02:awesome-event:20240310
1) "user_1"
2) "user_2"
3) "user_3"
...
# 해당 키에 속한 멤버 갯수를 리턴
> scard user02:awesome-event:20240310
(integer) 3
...
```sh
> SET user02:auth-text:user_1 1 NX EX 60
OK # 새로 저장된 Key인 경우 OK

> SET user02:auth-text:user_1 1 NX EX 60
(nil) # 기존에 저장된 Key인 경우 nil(null)
 
> SET user02:auth-text:user_2 1 NX EX 60
OK

> SET user02:auth-text:user_3 1 NX EX 60
OK
```
...
```sh
# user_1이 100점을 얻음
> ZINCRBY user02:ranking 100 user_1
"100"


# user_1이 50점을 얻음
> ZINCRBY user02:ranking 50 user_1
"150"

# user_2가 60점을 얻음
> ZINCRBY user02:ranking 60 user_2
"60"

# user_3이 200점을 얻음
> ZINCRBY user02:ranking 200 user_3
"200"

# user_4가 20점을 얻음
> ZINCRBY user02:ranking 20 user_4
"20"

# user_5가 20점을 얻음
> ZINCRBY user02:ranking 20 user_5
"20"
 
# 상위 3등의 데이터를 가져옴
> ZREVRANGE user02:ranking 0 2 WITHSCORES
1) "user_3"
2) "200"
3) "user_1"
4) "150"
...
```sh
> ZREVRANK user02:ranking user_3
(integer) 0 # 1등
 
> ZREVRANK user02:ranking user_2
(integer) 2 # 3등
 
> ZREVRANK user02:ranking user_1
(integer) 1 # 2등
```
...
> SADD user02:like:post:1 user_1
(integer) 1

# 1번 게시물에 user_3이 좋아요를 함
> SADD user02:like:post:1 user_3
(integer) 1
...
> SADD user02:like:post:2 user_1
(integer) 1

# 2번 게시물에 user_1이 좋아요를 취소함
> SREM user02:like:post:2 user_1
(integer) 1
...
> 
SADD user02:like:post:1 user_1
SADD user02:like:post:1 user_2
SADD user02:like:post:1 user_5
SADD user02:like:post:1 user_344


 
# 좋아요를 한 유저 조회
> SMEMBERS user02:like:post:1
1) "user_1"
2) "user_3"
3) "user_2"
4) "user_5"
...
# 2번 게시물에 유저들의 좋아요 입력
> 
ZADD user02:like:post:2 20240310213905 user_1
ZADD user02:like:post:2 20240310213915 user_2
ZADD user02:like:post:2 20240310214045 user_42
...
# 3번 게시물에 유저들의 좋아요 입력
> 
SADD user02:like:post:3 user_1
SADD user02:like:post:3 user_2
SADD user02:like:post:3 user_5
SADD user02:like:post:3 user_344


# 3번 게시물의 좋아요 개수 조회
> SCARD user02:like:post:3
(integer) 4
...
> ZADD user02:latest-seen-products:user_1 20240309142903 product_253
(integer) 1

# user_1이 2024-03-09 14:31:53에 product_153를 조회함
> ZADD user02:latest-seen-products:user_1 20240309143153 product_153
(integer) 1

# user_1이 2023-09-18 10:30:30에 product_685를 조회함
> ZADD user02:latest-seen-products:user_1 20240309103030 product_685
(integer) 1
 
# user_1의 조회한 상품 중 최근 10개만 조회
> ZREVRANGE user02:latest-seen-products:user_1 0 9
1) "product_685"
2) "product_153"
3) "product_253"
...
> ZADD user02:latest-seen-products:user_1 20240309142903 product_10
QUEUED
 
# 최근 본 상품 50개만 남기고 삭제
# start: 0, stop: -(남기고자하는 갯수 + 1)
> ZREMRANGEBYRANK user02:latest-seen-products:user_1 0 -51
QUEUED
