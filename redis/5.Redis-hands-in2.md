...
Cluster 내부에서 ds-redis-redis-cluster.redis-cluster.svc:6379 으로 접근 할것이다.
...
# LPUSH를 통한 list 생성
127.0.0.1:6379> LPUSH user01:myList "a"
(integer) 1
127.0.0.1:6379> LRANGE user01:myList 0 -1       # list 에 모든 값 확인
1) "a"

# LPUSH , RPUSH를 통한 요소 삽입 결과.
127.0.0.1:6379> LPUSH user01:myList "b"
(integer) 2
127.0.0.1:6379> RPUSH user01:myList "c"
(integer) 3
127.0.0.1:6379> LRANGE user01:myList 0 -1
1) "b"
2) "a"
3) "c"

127.0.0.1:6379> LPUSH user01:myList "d"
(integer) 4


# 기존에 있던 myList요소들은 [ d , b , a , c ] 순으로 되어있음.
# LPOP , RPOP을 통해 맨 좌 우측 요소 한개씩 제거 
127.0.0.1:6379> LPOP user01:myList
"d"
127.0.0.1:6379> RPOP user01:myList
"c"
127.0.0.1:6379> LRANGE user01:myList 0 -1
1) "b"
2) "a"
...
# 현재 List의 요소 길이를 출력.
127.0.0.1:6379> LLEN user01:myList
(integer) 2

# LREM을 통해 해당 요소 삭제, count를 0 으로 해서 요소중에 "a"랑 매칭되는 값을 삭제
127.0.0.1:6379> LREM user01:myList 0 "a"
(integer) 1
127.0.0.1:6379> LRANGE user01:myList 0 -1
1) "b"
...
127.0.0.1:6379> LSET user01:myList 0 "z"
OK
127.0.0.1:6379> LRANGE user01:myList 0 -1
1) "z"

# RPOPLPUSH 
127.0.0.1:6379> RPOPLPUSH user01:myList user01:hello     # myList 에서 rpop 하여 hello 에 lpush 한다.
"z"
127.0.0.1:6379> LRANGE user01:hello 0 -1
1) "z"
```
...
```sql
#  member 추가, 추가된 갯수 반환
127.0.0.1:6379> sadd user01:myset a
(integer) 1

127.0.0.1:6379> sadd user01:myset a
(integer) 0

127.0.0.1:6379> sadd user01:myset b
(integer) 1

127.0.0.1:6379> sadd user01:myset c
(integer) 1

# member 삭제, 삭제된 갯수 반환
127.0.0.1:6379> srem user01:myset c 
(integer) 1

127.0.0.1:6379> smembers user01:myset
1) "b"
2) "a"

127.0.0.1:6379> scard user01:myset
(integer) 2

# 여러 member 삽입 가능
127.0.0.1:6379> sadd user01:myset c d e f 
(integer) 4
...
# 랜덤 member 읽기 및 삭제
127.0.0.1:6379> spop user01:myset 3
1) "c"
2) "d"
3) "f"
...
127.0.0.1:6379> zadd user01:fruit 2 apple
(integer) 1

127.0.0.1:6379> zadd user01:fruit 10 banana
(integer) 1

# 복수개의 score-member를 추가할 수 있음
127.0.0.1:6379> zadd user01:fruit 8 melon 4 orange 6 watermelon
(integer) 3

# 이미 추가 된 member를 add 시 score가 업데이트
127.0.0.1:6379> zadd user01:fruit 15 apple
(integer) 0

# ZSCORE : member에 해당하는 score 값 리턴
127.0.0.1:6379> zscore user01:fruit apple
"15"

# ZRANK : member에 해당하는 rank(순위) 값 리턴
127.0.0.1:6379> zrank user01:fruit melon
(integer) 2

# ZRANGE : key에 해당하는 start - stop 내가 출력하고 싶은 요소를 추출
127.0.0.1:6379> zrange user01:fruit 0 -1
1) "orange"
2) "watermelon"
3) "melon"
4) "banana"
...
127.0.0.1:6379> hset user01:hh name song year 1975 month 1
(integer) 3

127.0.0.1:6379> hget user01:hh name
"jinmin"

127.0.0.1:6379> hget user01:hh year
"1995"

127.0.0.1:6379> hdel user01:hh year
(integer) 1

127.0.0.1:6379> hlen user01:hh
(integer) 2

127.0.0.1:6379> hgetAll user01:hh
1) "name"
2) "song"
3) "month"
4) "1"
...
127.0.0.1:6379> hkeys user01:hh
1) "name"
2) "month"


127.0.0.1:6379> hvals user01:hh
1) "song"
2) "1"
